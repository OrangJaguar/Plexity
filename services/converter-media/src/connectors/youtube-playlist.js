...
export const provider = 'youtube-playlist';
...
function isPlaylistUrl(parsed) {
  ...
  if (path === '/watch' && parsed.searchParams.has('list') && !parsed.searchParams.get('v')) return true;
  return path.includes('/playlist') || (parsed.searchParams.has('list') && path !== '/watch');
}

function isAllowedPlaylistMode(parsed) {
  const path = parsed.pathname.toLowerCase();
  if (path.includes('/playlist')) return true;
...
async function runYtdlpFlatPlaylist(url, maxItems) {
  const args = [
    '--flat-playlist',
    '--no-download',
    '--dump-single-json',
...
export async function discover(url, opts = {}) {
  ...
  if (!isAllowedPlaylistMode(parsed)) {
    const err = new Error('URL is not an allowlisted playlist mode');
    err.code = 'PROVIDER_UNSUPPORTED';
    throw err;
  }

  const maxItems = Math.min(opts.maxItems ?? QUOTAS.maxDiscoveryItems, QUOTAS.maxDiscoveryItems);
  const data = await runYtdlpFlatPlaylist(parsed.href, maxItems);
  ...
  const items = slice.map((entry, idx) => {
    ...
    return {
      ...
      sourceUrl: watchUrl,
      playlistIndex: idx + 1,
      metadata: { provider: provider, index: idx + 1 },
    };
  });
...
export async function resolve(url) {
  const err = new Error('Use discover() for playlists');
  err.code = 'PROVIDER_UNSUPPORTED';
  throw err;
}
