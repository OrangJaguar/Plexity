/**
 * tools-market-data — Yahoo Finance proxy (ported from Base44 toolsMarketData).
 */
import { jsonResponse, optionsResponse } from '../_shared/cors.ts';

const YAHOO_HOSTS = ["https://query1.finance.yahoo.com", "https://query2.finance.yahoo.com"];

const DEFAULT_UA = "Mozilla/5.0 (compatible; Plexity/1.0; +https://plexity.app)";
...
function marketDataUserAgent(): string {
  ...
  const contact = Deno.env.get("APP_CONTACT_URL")?.trim() || "https://plexity.app";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "yahoo") {
      ...
      if (!path) return jsonResponse({ error: { message: "Missing path" } }, 400);
